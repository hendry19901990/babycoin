...
The RPM spec file provided here is for Babycoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://babycoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Babycoin git repository](https://github.com/babycoin/babycoin) but are not part of
...
Currently only `Source10` is used, for the example `babycoin.conf` file.
...
in the babycoin git repository. At present only an SVG version of the Babycoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the babycoin
...
needed when 0.12.1 is released, a proper fix is already in the Babycoin git
master branch.
...
idea to build Babycoin Core against a static version of that library compiled
...
The `babycoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `babycoin-server` package. At this point, most RPM based
...
The `babycoin-server` package should have SELinux support. How to properly do
...
Babycoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/babycoin/). He has been
packaging Babycoin for Fedora at least since 2012.
...
4. I renamed the `babycoin` package that contains the Qt GUI to `babycoin-core` as
that appears to be how the general population refers to it, in contrast to
`babycoin-xt` or `babycoin-classic`. I wanted to make sure the general population
