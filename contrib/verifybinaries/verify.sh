...
###   babycoincore.org and babycoin.org and compares them.
...
WORKINGDIR="/tmp/babycoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://babycoincore.org"
HOST2="https://babycoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="babycoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'babycoin-',
...
if [ $? -ne 0 ]; then
   echo "babycoin.org failed to provide signature file, but babycoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "babycoin.org and babycoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Babycoin Core binary release signing key installed?"
   fi
