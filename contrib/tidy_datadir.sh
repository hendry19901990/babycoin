#!/bin/bash
# Copyright (c) 2013 The Babycoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Babycoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Babycoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Babycoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Babycoin 0.7 datadir."
    ;;
  3)
    echo "Detected Babycoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Babycoin 0.8 datadir."
    ;;
esac
